...
**关于流量异常的问题：这是由于软件没有成功获取到你的唯一识别码，导致你和其他一样没有唯一识别码的用户共用了流量。原因有可能是你没有允许本软件获取存储空间的权限，所以一定允许储存权限。处理办法请先清空本软件的全部数据或者卸载重装，然后允许获取存储空间的权限（允许权限最关键）。获取存储空间的权限只是为了写入闪退和错误日志，不会再有其他任何涉及隐私的行为，若您介意，请卸载。
